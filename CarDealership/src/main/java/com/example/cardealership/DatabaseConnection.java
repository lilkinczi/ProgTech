...
public class DatabaseConnection {
    ...
    public static void saveOrder(Order order) {
        String sql = "INSERT INTO orders (id, auto_id, napfenyteto, automata_valto, 2zonas_klima, ulesfutes, tolatokamera, szin, teljes_ar) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, order.getUserId());
            stmt.setInt(2, order.getCar().getAuto_Id());
            stmt.setBoolean(3, order.isNapfenyteto());
            stmt.setBoolean(4, order.isAutomataValto());
            stmt.setBoolean(5, order.isZonasKlima());
            stmt.setBoolean(6, order.isUlesfutes());
            stmt.setBoolean(7, order.isTolatokamera());
            stmt.setString(8, order.getSzin());
            stmt.setDouble(9, order.getTeljesAr());
            stmt.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public static List<Order> getOrders() {
        ...
        String sql = "SELECT o.*, c.gyarto, c.nev, c.tipus, c.ar FROM orders o JOIN auto c ON o.auto_id = c.auto_id";
        try (Connection conn = getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                Car car = carFactory.createCar(rs.getInt("auto_id"), rs.getString("gyarto"), rs.getString("nev"), rs.getString("tipus"), rs.getDouble("ar"));
                Order order = new Order(rs.getInt(getUserID(currentuser)), car, rs.getBoolean("napfenyteto"), rs.getBoolean("automata_valto"),
